...
int main()
{
	...
	scoreBg.setPosition(WINDOW_WIDTH - scoreBg.getGlobalBounds().width - 20, 20.0);
	...
	level.setPosition(WINDOW_WIDTH / 2, 20.0);
	...
	Score score(&scoreTexture, sf::Vector2u(10, 1), &digitTexture, sf::Vector2u(10, 1),
		scoreBg.getGlobalBounds().width, scoreBg.getPosition(), level.getGlobalBounds().width, level.getPosition());
			...
			if (inLunge)
			{
				...
				for (size_t i = 0; i < bugCount; ++i) // check every bug
				{
					...
					if (delayed && webStrands[spider.getRow()].intersects(bugRect)) // bug and spider are on the same strand, spider has reached 4th frame
					{
						...
						cout << "BUG HEALTH --------------> " << bugHealth << endl << endl << endl; 
																 // call destructor on bug when spider is in fifth frame
						score.updateScore(bugHealth, 2000);
					}
				}
